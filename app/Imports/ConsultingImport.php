...
class ConsultingImport implements ToCollection, WithHeadingRow
{
    public $errors = [];
    ...
    public function collection(Collection $rows)
    {
        ...
        // Get actual headers
        $actualHeaders = $firstRow->keys()->toArray(); // safer than array_keys($firstRow->toArray())

        // dd($rows->first()->keys(),$firstRow,$requiredHeaders,$actualHeaders);
        ...
        if ($this->importedCount == 0 && $hasRowData) {
            $this->errors[] = ['row' => '-', 'message' => 'No valid rows found to import.'];
        }
    }
