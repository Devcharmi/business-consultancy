var objective_table = $(".table-list").DataTable({
    order: [[0, "desc"]],
    autoWidth: false,
    processing: true,
    serverSide: true,
    ...
    ajax: {
        url: $("#objective_table").attr("data-url"),
    },
    columns: [
        {
            data: "id",
        ...
        },
        {
            ...
            mRender: function (v, t, o) {
                ...
                edit_path_set = edit_path_set.replace(":objective_manager", id);
...
$(document).on("click", "#objective_form_button", function () {
    let form = $("#objective_form");
    let url = $(this).attr("data-url");
    let method = form.find("input[name='_method']").length ? "PUT" : "POST"; // Detect if it's an update
    $("#objective_form").ajaxSubmit({
        url: url,
        type: method,
        dataType: "json",
        headers: {
