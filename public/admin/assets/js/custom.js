...
$(document).on("click", ".dropdown-item-close1", function (e) {
    e.preventDefault();
    e.stopPropagation();
    ...
    $.ajax({
        ...
        type: "GET",
        success: function () {
            liItem.fadeOut(300, function () {
                $(this).remove();

                const remaining = $(".dropdown-item-close1").length;
                $("#notification-data").text(`${remaining} Unread`);

                // Hide pulse icon when no unread left
                if (remaining === 0) {
                    $(".header-icon-pulse").addClass("d-none");
